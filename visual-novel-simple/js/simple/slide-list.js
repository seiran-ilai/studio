...
function renderSimpleSlideList() {
  ...
  cards.forEach((card, idx) => {
    ...
    // 任務 2:每張幕(含當前 / 最後一張)都可刪除;刪最後一張會跳「清空並回到初始」警告。
    ...
    del.addEventListener("click", (e) => {
      e.stopPropagation();
      deleteSimpleSlideWithConfirm(card.id);
    });
...
function _doSelectSimpleSlide(slideId) {
  ...
  saveToStorage();
  // 任務 2-3:即使某幕資料異常(例如引用的 CG 已被刪),也不要中斷整個工具
  try {
    renderSimpleSlideList();
    renderSimpleEditor();
  } catch (err) {
    console.error("切幕渲染失敗:", err);
    if (typeof showToast === "function") showToast("切幕時發生錯誤,已略過", "warn");
  }
}
...
// 任務 2:刪除指定幕(含確認)。最後一張 → 特別警告「清空並回到初始」,確認後回到空狀態。
// CG 庫與對話框樣式設定都保留(deleteSimpleSlide 只動 simpleCards)。
async function deleteSimpleSlideWithConfirm(slideId) {
  const cards = state.simpleCards || [];
  if (!cards.some(s => s.id === slideId)) return;
  const isLast = cards.length === 1;
  const ok = await inlineConfirm(isLast ? {
    title: "⚠ 刪除最後一張幕",
    message: "這是專案中最後一張幕。\n刪除後將回到初始狀態,劇本內容會清空。\n\n(CG 庫與樣式設定會保留)",
    okText: "清空並回到初始",
    cancelText: "取消",
  ...
  if (!ok) return;
  deleteSimpleSlide(slideId);
  if (!(state.simpleCards && state.simpleCards.length)) {
    showToast("✓ 已清空,可從上傳圖片開始", "success", 3000);
  } else {
    showToast("已刪除", "info", 1500);
  }
}
...
async function deleteCurrentSlideWithConfirm() {
  ...
  const idx = getCurrentSlideIdx();
  if (idx < 0) return;
  await deleteSimpleSlideWithConfirm(cards[idx].id);
}
